...
//    }
    ...
    // 일촌들 조회(그냥 친구목록 볼때, 순서 까지)
    @GetMapping("/family-wind")
    ResponseEntity<?> getFamiliesWind(@RequestParam Long userSeq) throws Exception{
        return ResponseEntity.ok().body(familyService.getFamiliesWind(userSeq));
    }
